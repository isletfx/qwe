			...
			<c:forEach var="p" items="${ pageBean.list }">
				<div class="col-md-2">
					<a href="${pageContext.request.contextPath }/ProductServlet?method=findByPid&pid=${p.pid}">
						<img src="${pageContext.request.contextPath }/jsp/${p.pimage}" width="170" height="170" style="display: inline-block;">
					</a>
					<p><a href="${pageContext.request.contextPath }/ProductServlet?method=findByPid&pid=${p.pid}" style='color:green'>${ fn:substring(p.pname,0,8) }</a></p>
			...
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<li <c:if test="${ pageBean.currPage == 1 }">class='disabled'</c:if>>
					<c:if test="${ pageBean.currPage != 1 }">
						<a href="${pageContext.request.contextPath }/ProductServlet?method=findByCid&currPage=${ pageBean.currPage - 1 }&cid=${ param.cid}" aria-label="Previous">
			...
			<div style="overflow: hidden;">
				...
				<%
					Cookie[] cookies = request.getCookies();
									...
									ProductService productSerice = new ProductServiceImpl();
									if(cookie == null){
										//如果等于空,说明是第一次浏览
										out.println("<h3>你还没有浏览商品</h3>");
									}else{
										//不是第一次浏览
										String value = cookie.getValue();
										String[] ids = value.split("-");
										for(String id:ids){
											Product product = productSerice.findByPid(id);
											pageContext.setAttribute("p", product);
				%>
					<li style="width: 150px;height: 216;float: left;margin: 0 8px 0 0;padding: 0 18px 15px;text-align: center;"><img src="${pageContext.request.contextPath }/jsp/${p.pimage}" width="130px" height="130px" /></li>
				<% 
						}
					}
				%>
